    ...
    <div class="search">
        <div class="b_search">
            <div class="b_search2" style="width: 280px;">
                ...
                <span class="p_search"><a href="<c:url value="./workData" />">등록</a></span>
            </div>
        </div>
    </div>
    <div class="swrap">
        <table class="board_list" style="width:100%;">
            ...
            <tbody>
                ...
                <c:choose>
                    <c:when test="${!empty workDataList}">
		                <c:forEach items="${workDataList}" var="item">
			                <tr>
...
<script>
	...
	function init() {
        ...
        $(document).on('click', '.select', detailWorkData);
	...
	function goPage(pageNo) {
	    var url = '<c:url value="./workDataList" />';
   ...
   function detailWorkData() {
       var bodId = $(this).data('bodId');
       var url = '<c:url value="./workDataDetail" />' + '?bodId=' + bodId;
